...
import type { RatingTarget, RatingQuestion } from '@/types';
...
export default function AdminRatingsPage() {
  ...
  const handleTargetSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = editingTarget ? { ...targetForm, id: editingTarget.id } : targetForm;
      const res = await fetch('/api/admin/ratings', {
        method,
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">管理评分对象和评分题目</p>
          </div>
        ...
        {/* 评分对象列表 */}
        {targets.length === 0 ? (
          <div className="card text-center text-gray-500 py-12">暂无评分对象，点击上方按钮添加</div>
        ) : (
